...
| Answering "has this detection fired before?" from narrative memory | `grouped_details` on the detection carries one entry per occurrence, with per-occurrence timestamps and byte counts | Read `grouped_details` (via `get_detection_details`) instead of inferring recurrence from timestamps or prior conversation |
